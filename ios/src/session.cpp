...
static const olmKit::KdfInfo OLM_KDF_INFO = {
...
std::size_t olmKit::Session::new_outbound_session_random_length() const {
    return CURVE25519_RANDOM_LENGTH * 2;
}


std::size_t olmKit::Session::new_outbound_session(
    olmKit::Account const & local_account,
    ...
    olmKit::unset(base_key);
    olmKit::unset(ratchet_key);
    olmKit::unset(secret);
...
static bool check_message_fields(
    olmKit::PreKeyMessageReader & reader, bool have_their_identity_key
) {
    bool ok = true;
...
std::size_t olmKit::Session::new_inbound_session(
    olmKit::Account & local_account,
    _olm_curve25519_public_key const * their_identity_key,
    std::uint8_t const * one_time_key_message, std::size_t message_length
) {
    olmKit::PreKeyMessageReader reader;
    ...
    olmKit::load_array(alice_identity_key.public_key, reader.identity_key);
    olmKit::load_array(alice_base_key.public_key, reader.base_key);
    olmKit::load_array(bob_one_time_key.public_key, reader.one_time_key);

    olmKit::MessageReader message_reader;
    decode_message(
    ...
    olmKit::load_array(ratchet_key.public_key, message_reader.ratchet_key);

    olmKit::OneTimeKey const * our_one_time_key = local_account.lookup_key(
        bob_one_time_key
    );
...
std::size_t olmKit::Session::session_id_length() const {
    return SHA256_OUTPUT_LENGTH;
}


std::size_t olmKit::Session::session_id(
    ...
    std::uint8_t * pos = tmp;
    pos = olmKit::store_array(pos, alice_identity_key.public_key);
    pos = olmKit::store_array(pos, alice_base_key.public_key);
    pos = olmKit::store_array(pos, bob_one_time_key.public_key);
    _olm_crypto_sha256(tmp, sizeof(tmp), id);
    return session_id_length();
}


bool olmKit::Session::matches_inbound_session(
    _olm_curve25519_public_key const * their_identity_key,
    std::uint8_t const * one_time_key_message, std::size_t message_length
) const {
    olmKit::PreKeyMessageReader reader;
...
olmKit::MessageType olmKit::Session::encrypt_message_type() const {
    if (received_message) {
        return olmKit::MessageType::MESSAGE;
    } else {
        return olmKit::MessageType::PRE_KEY;
    }
}


std::size_t olmKit::Session::encrypt_message_length(
...
std::size_t olmKit::Session::encrypt_random_length() const {
    return ratchet.encrypt_random_length();
}


std::size_t olmKit::Session::encrypt(
    ...
    if (received_message) {
        ...
        olmKit::store_array(writer.one_time_key, bob_one_time_key.public_key);
        olmKit::store_array(writer.identity_key, alice_identity_key.public_key);
        olmKit::store_array(writer.base_key, alice_base_key.public_key);
...
std::size_t olmKit::Session::decrypt_max_plaintext_length(
    MessageType message_type,
    std::uint8_t const * message, std::size_t message_length
) {
    std::uint8_t const * message_body;
    std::size_t message_body_length;
    if (message_type == olmKit::MessageType::MESSAGE) {
        message_body = message;
...
std::size_t olmKit::Session::decrypt(
    olmKit::MessageType message_type,
    std::uint8_t const * message, std::size_t message_length,
...
void olmKit::Session::describe(char *describe_buffer, size_t buflen) {
...
std::size_t olmKit::pickle_length(
    ...
    std::size_t length = 0;
    length += olmKit::pickle_length(SESSION_PICKLE_VERSION);
    length += olmKit::pickle_length(value.received_message);
    length += olmKit::pickle_length(value.alice_identity_key);
    length += olmKit::pickle_length(value.alice_base_key);
    length += olmKit::pickle_length(value.bob_one_time_key);
    length += olmKit::pickle_length(value.ratchet);
    return length;
}
...
std::uint8_t * olmKit::pickle(
    ...
    pos = olmKit::pickle(pos, SESSION_PICKLE_VERSION);
    pos = olmKit::pickle(pos, value.received_message);
    pos = olmKit::pickle(pos, value.alice_identity_key);
    pos = olmKit::pickle(pos, value.alice_base_key);
    pos = olmKit::pickle(pos, value.bob_one_time_key);
    pos = olmKit::pickle(pos, value.ratchet);
    return pos;
}
...
std::uint8_t const * olmKit::unpickle(
    ...
    pos = olmKit::unpickle(pos, end, value.received_message); UNPICKLE_OK(pos);
    pos = olmKit::unpickle(pos, end, value.alice_identity_key); UNPICKLE_OK(pos);
    pos = olmKit::unpickle(pos, end, value.alice_base_key); UNPICKLE_OK(pos);
    pos = olmKit::unpickle(pos, end, value.bob_one_time_key); UNPICKLE_OK(pos);
    pos = olmKit::unpickle(pos, end, value.ratchet, includes_chain_index); UNPICKLE_OK(pos);
